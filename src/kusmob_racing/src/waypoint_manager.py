...
def callback(data):
    ...
    with open('waypoint.csv', 'a') as p: # csv파일에 waypoint 행을 append 함
        wr = csv.writer(p)
        wr.writerow([pos.position.x,pos.position.y,pos.orientation.z,pos.orientation.w])
        global counter
        print("{}번째 waypoint saved {0},{1},{2},{3},").format(counter, pos.position.x,pos.position.y,pos.orientation.z,pos.orientation.w)
